...
def train_feat_syn(feat_syn,labels_syn,feat_train_batch,index,index_syn,knn_class,coeff,coeff_sum,args,device):
...
# todo MyWandb
# wandb.init(project="x.o_Cora数据集_<3",name=time.strftime("%Y,%m,%d-%H:%M:%S"),config=config_dict)
...
# Step 6: 优化合成特征
train_feat_syn(feat_syn_param,labels_syn,feat_train,index,index_syn,knn_class,coeff,coeff_sum,args,device)
...
if args.generate_adj == 1: # (A',X')
    ...
    graph = Data(x=feat_syn.data, y=labels_syn, edge_index=adj.nonzero().t(),
    ...
    # (I,X')
    graph = Data(x=feat_syn.data, y=labels_syn, edge_index=torch.eye(len(feat_syn.data)).nonzero().t(),
...
for repeat in range(args.repeat):
    ...
    acc.append(model_training(model, args, ori_data, graph, data_val, data_test, wandb))
# result_record(args, acc)
print(acc)


# todo 记录数据
csv_logger = TrainingLogger('./res/826AblationTest/noMlp.csv', mode='csv')
